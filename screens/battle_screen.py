...
from screens.bullet_board import BulletBoard
...
class BattleScreen(Screen):
    ...
    def __init__(self, boss_id=None):
        ...
        if boss_id:
            from assets.monsters_data import MONSTER_LIST
            self.monster = next((m for m  in MONSTER_LIST if m["id"] == boss_id), None)
        else:
            pass
        # load stats
    ...
    def on_mount(self): #amazing!
        if hasattr(self, "monster") and self.monster is not None:
            self.current_monster = self.monster.copy()
            self.current_monster["max_hp"] = self.current_monster["hp"]
            self.update_monster_visuals()
            
            box = self.query_one("#dialogue-box")
            box.update(f"* {self.current_monster['name']} blocks the way!")
        else:
            self.load_random_monster()

        play_music("battle")
        
        # 3. 🚨 ACTUALLY START THE BATTLE!
        self.set_timer(1.0, self.start_player_turn)
    ...
    def start_player_turn(self):
        ...
        self.turn_count += 1 #neede to fix
    ...
    def enemy_turn(self):
        ...
        def handle_defense(hits_taken):
            ...
            final_damage = int(raw_damage * hits_taken)
            ...
            if final_damage == 0:
                ...
                msg = f"* You got hit {hits_taken} times! Took {final_damage} damage!"
            box.update(msg)
        ...
        dynamic_time = self.current_monster.get("attack_duration", max(3.0, self.current_monster["atk"] * 0.4))
        final_duration = min(dynamic_time, 8.0)
        # def start
        self.set_timer(0.1, lambda: self.app.push_screen(BulletBoard(duration=final_duration), handle_defense))
    ...
    def handle_buttons(self, event: Button.Pressed):
        ...
        if btn_id == "btn-fight": #rev 2
            ...
            self.set_timer(0.1, lambda: self.app.push_screen(TimingGame(
